...
async function fetchUserData() {
  ...
  let data = await fetch("https://api.example.com/data"); // 시간을 걸리는 작업 + await(기다림)
...
async function fetchCustomerData() {
  try {
    ...
    if (!data.ok) {
      throw new Error(
        "네트우크 응답이 실패되어 데이터를 정상적으로 가져오지 못하였습니다."
      );
    }
...
// 4) headers
// : 응답 헤더를 나타냄, 응답의 메타데이터나 특정 값을 포함

// 5) body
// : 응답의 본문, 해당 데이터를 .json()을 사용으로 data 추출