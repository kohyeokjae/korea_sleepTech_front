...
// EX) 사용자 이름 쿠키 설정
// : "username"의 이름에 "HGD" 값을 저장

document.cookie = "username=hgd; path=/;";

// cf) 경로: 해당 웹 페이지에서 쿠키값에 접근할 수 있는 경로를 지정
//     >> '/' 빈 슬래시 사용: 어떤 경로에서는 쿠키값에 접근 가능

// 1) 도메인
// : 웹 사이트의 주소 "naver.com", "google.com" 등의 서버 위치(IP 주소)를 식별하는 역할
// 2) 경로
// : 도메인 두에 이어지는 부분, 서버(사이트) 내의 특정 파일이나 페이지 위치를 나타냄
// "naver.com/news", "google.com/email"

//? 2) 만료 날짜 설정
// : expires 속성을 설정
// - 기본값(설정 X): 세션 쿠기로 자동 설정되어 브라우저가 닫힐 때 자동 삭제
