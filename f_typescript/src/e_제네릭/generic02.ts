...
//   console.log(arg.length); // 'T' 형시에 'length' 속성이 없습니다.
// }
...
console.log(
  constraints({
    length: 10, // 필수 속섬만 명시 되어 있으면 가능 - length 속성을 반드시 포함 (구조적 타이핑)
    value: 3,
    addedOption: "hi",
  })
);
...
type Check<T> = T extends string ? "String" : "Not a String"; // 타입에 따라 리터럴 타입으로 반환
