...
function updateUser(id: number, changes: Partial<User>) {
  ...
  // 2) age만 있는 경우: { name, age, name } >> changes의 age로 값이 수정
...
updateUser(1, { name: "충길동" });
console.log(users[1]); // { name: '충길동', age: 30 }
updateUser(2, { age: 30 });
console.log(users[2]); // { name: '고길동', age: 30 }

updateUser(3, { name: "구길동" }); // 해당 id의 사용자는 없습니다.

updateUser(1, { name: "옹길동", age: 49 });
console.log(users[1]); // { name: '옹길동', age: 49 }
...
// user.name = "고길동"; // 읽기 전용 속성이므로 'name'에 할당할 수 없습니다.
// user.age = 39;
