{
  ...
  const fetchUsers = async (): Promise<UsersType> => {
    ...
    //   , 자동으로 Promise.resolve(어떤값)로 감싸서 반환
  ...
  //   , 사용자의 이름, 사용자면, 이메일 중 하나라도 포함된 경우 출력
  // @Params: users - UsersType 타입 (전체 사용자들 데이터)
  //          , query - string 타입 (필터링할 검색어)
  // @Return: UsersType 타입 (필터링 된 사용자 데이터)
  ...
  const sortUsers = (users: UsersType, key: "name" | "email"): UsersType => {
    ...
    //? [...users]
    // : 새로운 배열 주소값에 기존의 사용자 데이터(요소)만 복사해서 가저옴
    // - 깊은 복사
    // - 배열의 불변성을 보완

    //? 배열.sort()
    // : 배열의 요소를 정렬할 때 사용
    // - 콜백 함수를 인자로 받음 (콜백 함수의 인자는 비교할 데이터를 2가지 입력)
  ...
  //! 9) 초기화 함수
  // : 사용자 데이터를 가져와 하면에 표시 + 이벤트 리스너를 설정
