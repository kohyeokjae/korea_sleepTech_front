...
// cf) Link 컴포넌트와의 차이
// : useNavigate() 훅은 이벤트 핸들러나 비동기 작업 내에서 특정 경로 이동이 가능
...
function UseNavigate() {
  ...
  const goToOtherPage = () => {
    //! useNavigate 훅의 옵션
    // - state 옵션
    // : 네비게이션(경로)과 함께 상태 전달 기능
    // : useLocation() 훅을 통해 접근 기능 (A 컴포넌트)
    navigate("/router/a", { state: { userId: 1234 } });
  };
