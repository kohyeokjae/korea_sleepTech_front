...
function LoginPage() {
  ...
  // input 요소에 접근(username, password)
  ...
  const handleInputOnKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key == "Enter") {
      ...
      let foundIndex = inputRefs.findIndex(
        (ref) => ref.current === e.currentTarget
      );

      if (foundIndex === inputRefs.length - 1) {
        // 찾은 요소가 마지막 input 창일 경우 버큰 클릭 실행
  ...
  const handleLoginSubmitOnClick = async () => {
    try {
      const response = await axios.post(
        "https://localhost:8080/spring_boot_app/api/login",
        inputValue
      );
      ...
      console.error("로그인 실패(존재하지 않은 api URL): ", error);
      alert("로그인 실패");
    }
  };
