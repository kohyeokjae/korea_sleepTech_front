...
/*
  ! === 부수 효과 (Side Effect) ===
  : 컴포넌트 '주요 기능(상태 관리, UI 렌더링)' 외에 발생하는 작업
  EX) API 호출, 이벤트 리스너 등록, 수동 DOM 조작 등

  ! useEffect
  : React 함수형 컴포넌트에서 부수 효과를 수행하기 위한 Hook
  - 데이터를 가져오기, 컴포넌트 렌더링 시 특정 작업 수행 등에 사용

  ? React 컴포넌트의 '라이프 사이클'
  1) 마운팅: 컴포넌트가 DOM에 처음 삽입될 때 살행
  ...
  useEffect(() => {
    ...
    ? useEffect의 정리 함수
    : useEffect에서 함수를 return하면 함수는 
      컴포넌트가 화면에서 사라질 때 (언마운트) 실행
...
function UseEffect01() {
  ...
  return (
    <div>
      ...
      */}
      <button onClick={() => setName(name === "홍길동" ? "고길동" : "홍길동")}>
        이름 변경
      </button>
